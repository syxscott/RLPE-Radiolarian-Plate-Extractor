...
> ⚠️ **These F1 figures are NOT ground-truth accuracy.** The 9-paper
> gold set was generated by RLPE's own caption parser
> (`scripts/build_gold_from_captions.py`), so the numbers below measure
> **parser self-consistency** (how well one pipeline stage reproduces
> another), not extraction accuracy against manually annotated panel
> images. A 2026-09-02 image-verified re-measurement of an earlier
> "0.84 F1" checkpoint put true image-verified F1 at **0.075** — treat
> all parser-derived F1 in this file as an upper bound only. See
> `src/rlpe/evaluation/gold.py` (module docstring) and the
> `gold_provenance` field now embedded in every evaluation report.
> Publishing any F1 claim requires a human image-verified gold set.
...
注意：gold 为 caption parser 自产，此数字衡量 parser self-consistency，
不是 image-verified 精度，见上方 2026-08-02 段落的警告）、
...
# 注意：此 gate 的 gold 为 parser 自产，F1 衡量 self-consistency；
# image_label_match 才是接近 image-verified 的信号，不要只看 F1。
