...
- **Aggregate species F1: 93.06%** (precision 93.06%, recall 93.06%)
- **Aggregate panel match rate: 100.00%** (every gold panel has a
  matching prediction row)
- **Aggregate exact-match rate: 93.06%** (both panel and species correct)
- **318 tests passing, 2 skipped** (`python -m pytest tests/`)
- **v9 → v12 trajectory**: 71.5% → 88.82% → 91.31% → 92.91% → **93.06%** F1
  (+21.6pp over three parser+normalization+figure-id rounds)
...
| **Aggregate** | **519** | | **100%** | **93.06%** | **93.06%** | **93.06%** | **93.06%** |
