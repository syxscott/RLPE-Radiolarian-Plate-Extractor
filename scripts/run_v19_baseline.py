...
from prompts import select_prompt, build_user_prompt  # noqa: E402
...
V19_PAPERS = [
    'bandini2011', 'baumgartner2008', 'beccaro2006', 'boughdiri2007',
...
def micro_f1_with_ci(preds, gold, n_bootstrap: int = 1000):
    ...
    def _f1(pp, gp):
        ...
        for pp_, gp_ in by_paper.values():
            pset = {(_norm_species(x.get('species')), x.get('figure_id'), x.get('panel_id')) for x in pp_ if x.get('species')}
            gset = {(_norm_species(x.get('species')), x.get('figure_id'), x.get('panel_id')) for x in gp_ if x.get('species')}
            tp += len(pset & gset)
            fp += len(pset - gset)
            fn += len(gset - pset)
        if tp == 0:
    ...
    papers = sorted({(p.get('paper_id'), g.get('paper_id')) for p in preds for g in gold if p.get('paper_id') and g.get('paper_id') and p.get('paper_id') == g.get('paper_id')})
    if not papers:
        return point, (point, point)
...
def main():
    ...
    parser.add_argument('--rate-limit-sec', type=int, default=60,
                        help='Sleep between papers (default 60s — matches run_research_eval.py)')
    parser.add_argument('--dry-run', action='store_true',
                        help='Print plan and exit without calling the API')
