...
class _StubPipeline:
    ...
    def __init__(self, cfg: PipelineConfig):
        ...
        self.config = cfg
        # Provide a stub semantic_engine with a non-None backend so the method's
        # second guard (semantic_engine.backend) does NOT short-circuit and the
        # actual Task 3 loop body gets exercised by skip-rows tests.
        self.semantic_engine = SimpleNamespace(backend=SimpleNamespace(backend_name="stub"))
        ...
        self._apply_llm_per_panel_species_id = (
            RadiolarianPipeline._apply_llm_per_panel_species_id.__get__(self, RadiolarianPipeline)
        )
...
def test_config_post_init_validates_conf_range(tmp_path):
    cfg = _make_cfg(tmp_path, llm_per_panel_min_conf=1.5)
    with pytest.raises(ValueError, match=r"llm_per_panel_min_conf must be"):
        cfg.__post_init__()


def test_config_post_init_validates_caps_positive(tmp_path):
    cfg = _make_cfg(tmp_path, llm_per_panel_max_per_figure=0)
...
def test_method_overwrites_species_when_llm_high_confidence(tmp_path):
    ...
    confidence ``>= llm_per_panel_min_conf``, the row's species is
    overwritten."""
    cfg = _make_cfg(
        tmp_path,
        llm_per_panel_enabled=True,
...
def test_method_keeps_regex_when_llm_returns_empty_species(tmp_path):
    """Regression: when LLM returns confidence >= min_conf but with
    ...
    at pipeline.py:2348 protects against LLM hallucinating an empty
    ...
    # Same fixture as test_method_overwrites_species_when_llm_high_confidence,
    ...
    assert backend.infer_panel.called
    assert "llm_per_panel" in out[0]["metadata"]
    assert out[0]["metadata"]["llm_per_panel"]["confidence"] == pytest.approx(0.95)
...
def test_cli_argparse_accepts_llm_per_panel_flags(tmp_path):
    ...
    ``--no-llm-per-panel`` deliberately *shares* ``dest="llm_per_panel"``
    with ``--llm-per-panel`` (so the last flag on the command line wins),
    which means no ``no_llm_per_panel`` identifier exists to grep for.
    """
    import inspect
    ...
    assert defaults.llm_per_panel is False
    assert defaults.llm_per_panel_min_conf == 0.55
    assert defaults.llm_per_panel_max_per_figure == 20
    assert defaults.llm_per_panel_max_per_paper == 200
    ...
    assert on.llm_per_panel is True
    assert on.llm_per_panel_min_conf == 0.9
    assert on.llm_per_panel_max_per_figure == 3
    assert on.llm_per_panel_max_per_paper == 7

    # Shared dest => explicit opt-out wins when it comes last.
    assert (
        parser.parse_args(base + ["--llm-per-panel", "--no-llm-per-panel"]).llm_per_panel is False
    )
    ...
    assert "llm_per_panel_enabled=args.llm_per_panel" in src
    # The Stage 3 + multi-plate enrichment gates have the same defect;
    # both must be wired as typed kwargs in the CLI.
    assert "llm_stage3_enabled=bool(args.use_llm_stage3)" in src
    assert "llm_multi_plate_enrich_enabled=bool(args.llm_multi_plate_enrich)" in src


# ---------------------------------------------------------------------------
# Audit 2026-08-17: regression tests for the flag-wiring bug.
#
# Pre-fix: CLI set ``llm_per_panel_enabled`` as a typed PipelineConfig
# attribute, but the Stage 4.5 gate at pipeline.py line 1722 read it from
# ``self.config.extra.get("llm_per_panel_enabled", False)`` which the CLI
# never populated. Live smoke on Bandini 2011 (commit 654c0fc) showed
# 0/64 rows reached Stage 4.5 even with ``--llm-per-panel``. The same
# defect affected the ``llm_stage3`` / ``llm_multi_plate_enrich`` gates.
#
...
def test_config_has_llm_gate_typed_attrs(tmp_path):
    """``PipelineConfig`` must expose the three LLM gate flags as typed
...
def test_pipeline_gates_use_typed_attrs_not_extra(tmp_path):
    """Source-guard regression for the flag-wiring bug. The three LLM
    ...
    src = inspect.getsource(RadiolarianPipeline)
    # The buggy pattern: ``self.config.extra.get("llm_per_panel_enabled", ...``.
    # After the fix, none of the three LLM gates should still use this
    # pattern.
    for forbidden in (
        'self.config.extra.get("llm_per_panel_enabled"',
        'self.config.extra.get("llm_stage3"',
        'self.config.extra.get("llm_multi_plate_enrich"',
    ):
    ...
    assert "self.config.llm_per_panel_enabled" in src
    assert "self.config.llm_stage3_enabled" in src
    assert "self.config.llm_multi_plate_enrich_enabled" in src
...
def test_stage4_5_gate_fires_with_typed_attr(tmp_path):
    ...
    return triggered and ``metadata.llm_per_panel`` was never stamped."""
    from types import SimpleNamespace
    ...
    pipe = _StubPipeline(cfg)
    pipe.semantic_engine = SimpleNamespace(backend=backend)
...
def test_cli_no_longer_mirrors_llm_per_panel_into_extra(tmp_path):
    ...
    assert 'cfg.extra["llm_per_panel_enabled"]' not in src, (
        "CLI must NOT mirror llm_per_panel_enabled into extra; gates "
        "now read the typed attribute directly."
    )
    assert 'cfg.extra["llm_per_panel_min_conf"]' not in src
    assert 'cfg.extra["llm_per_panel_max_per_figure"]' not in src
    assert 'cfg.extra["llm_per_panel_max_per_paper"]' not in src
...
# text was never sliced. Result: LLM received panels with no context and
...
#   1. caption_pairs is propagated into the LLM prompt as caption_for_panel
#   2. page_context_snippet is propagated into the LLM prompt body
#   3. empty context still calls LLM (no silent skip)
# ---------------------------------------------------------------------------
...
class _CaptureBackend:
    ...
    Inspect-mode: we don't actually run an LLM call; we capture the
    prompt / metadata the Stage 4.5 worker passes to the backend so the
    test can assert on the propagated context. The LLM response is a
...
def test_caption_pairs_plumbed_into_llm_prompt(tmp_path):
    """When a row carries a populated ``caption_pairs`` whose labels
    include the row's panel_id, the LLM prompt's caption block carries
...
def test_page_context_snippet_plumbed_into_llm_prompt(tmp_path):
    """When a row carries a populated ``page_context_snippet``, that
    snippet is rendered into the LLM prompt's '[Same-page context]' block.
    """
    from types import SimpleNamespace
...
def test_empty_context_still_calls_m3(tmp_path):
    ...
    ``page_context_snippet`` MUST still produce an LLM call (no silent
