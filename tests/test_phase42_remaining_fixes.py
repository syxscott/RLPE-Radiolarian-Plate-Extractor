...
def test_pipeline_worker_request_cancel_sets_event():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        w.request_cancel()
        assert w._cancel_event.is_set(), "request_cancel must set _cancel_event"
        assert calls == [True], "request_cancel must also call requestInterruption"
...
def test_to_jsonable_handles_datetime():
    ...
    dt = datetime.datetime(2026, 7, 13, 14, 30, 0, tzinfo=datetime.UTC)
    out = to_jsonable(dt)
...
def test_file_size_human_byte_unit():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert result == "999 B", f"999-byte file should be '999 B', got {result!r}"
...
def test_single_instance_lock_uses_fcntl_on_linux():
    ...
    with mock.patch.object(fcntl, "flock") as mock_flock:
        ...
        assert mock_flock.called, "fcntl.flock should be called on Linux"
...
def test_constants_ranges_validated_at_import():
    ...
    for name in dir(c):
        if not name.startswith("RANGE_"):
            continue
        val = getattr(c, name)
        assert isinstance(val, tuple) and len(val) == 2, f"{name} must be a 2-tuple, got {val!r}"
        lo, hi = val
...
def test_default_llm_backend_is_known():
    ...
    assert hasattr(c, "_KNOWN_LLM_BACKENDS"), "constants module should export _KNOWN_LLM_BACKENDS"
...
def test_main_window_batch_stop_on_error_pauses_batch():
    ...
    assert w._batch_pdfs == [], (
        f"_on_job_failed with stop_on_error should clear _batch_pdfs, got {w._batch_pdfs!r}"
    )
