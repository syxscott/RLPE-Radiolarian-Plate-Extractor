...
must NOT contain ``llm_api_key`` or any other API-key string.

Before: ``provenance.config_snapshot`` was the full
``PipelineConfig.model_dump()`` without redaction. If the user
started a run with ``llm_api_key=sk-ant-api03-abcdef...``,
...
API-key fields by name (``llm_api_key``,
``MiniMax_extra_headers``, ``_llm_external_handler``, ...),
...
class _FakeConfig:
    ...
    def __init__(
        ...
        _llm_external_handler: str = "sk-proj-" + "a" * 32,
        gemma_use_4bit: bool = True,
    ) -> None:
        self.llm_api_key = minimax_api_key
        self.llm_base_url = minimax_endpoint
        self.grobid_url = grobid_url
        self.prompt_with_key = prompt_with_key
        self._llm_external_handler = _llm_external_handler
...
def test_redact_api_keys_in_provenance_strips_known_fields():
    ...
    assert "llm_api_key" not in snap, (
        f"llm_api_key leaked into config_snapshot: {snap.get('llm_api_key')!r}"
    )
    # _llm_external_handler is private-prefixed; the fallback walker
...
def test_redact_api_keys_pseudo():
    ...
    cfg_dict = {
        "llm_api_key": "sk-ant-api03-" + "a" * 48,
        "llm_base_url": "https://api.example.com",
        "_llm_external_handler": "sk-proj-" + "a" * 32,
        "gemma_use_4bit": True,
        "nested": {
