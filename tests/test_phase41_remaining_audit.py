...
def test_run_tab_blocks_double_click_start():
    ...
    with (
        patch("rlpe.gui.run_tab.PipelineWorker", side_effect=fake_worker),
        ...
        try:
            rt._on_start()
            assert len(workers) == 1, f"First click should spawn 1 worker, got {len(workers)}"
...
def test_run_tab_status_label_uses_i18n():
    ...
    assert '"Cancelling…"' not in src, "run_tab still has bare English 'Cancelling…' string"
...
def test_batch_dialog_clear_all_empties_internal_list():
    ...
    dlg._on_clear_all()
    assert dlg._file_list.count() == 0, "QListWidget must be empty after _on_clear_all"
    assert dlg._pdfs == [], f"self._pdfs must be empty after _on_clear_all, got {dlg._pdfs!r}"
...
def test_batch_dialog_buttons_translate():
    ...
    assert not no_obj, f"Batch dialog has QPushButton without objectName (no i18n): {no_obj}"
...
def test_image_preview_set_image_clears_old_bboxes():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert len(w._bbox_items) == 1, (
            f"After set_bboxes([1 bbox]), _bbox_items should have 1 item, got {len(w._bbox_items)}"
        )
        ...
        assert len(w._bbox_items) == 0, (
            f"After set_bboxes([]), _bbox_items should be 0, got {len(w._bbox_items)}"
        )
