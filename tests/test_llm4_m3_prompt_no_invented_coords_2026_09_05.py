"""Regression: audit 2026-09-04 llm-4 — the LLM prompt few-shot
examples in :mod:`rlpe.semantic_engine` taught the model to INVENT
coordinates from locality / country strings:

  semantic_engine.py:438-439   Mt. Wilmot, Wyoming  →  44.0, -107.9
  semantic_engine.py:535-536   Lombardy Basin, Italy → 45.4, 9.5
  semantic_engine.py:649       Italy (filled circle) → 45.4, 12.0
  semantic_engine.py:656       Tunisia (filled circle) → 34.0, 9.0
  semantic_engine.py:663       Spain (filled circle) → 40.0, -3.0
...
Fix contract:
  ...
  * No more "model-trained-on-fabrication" pattern in the LLM prompt
    library.
"""
...
class TestM3PromptsDoNotTeachCoordinateInvention:
    ...
    def test_few_shot_block_for_locale_uses_null_lat(self, locale, fab_value):
        ...
        """
        src = (_SRC / "rlpe" / "semantic_engine.py").read_text(encoding="utf-8")
        idx = src.find(f'"{locale}"')
        if idx == -1:
            # Locale not in the file — fix already applied, nothing
            # to check. (Audit flagged it; fix removed it.)
            pytest.skip(f"{locale!r} no longer appears in semantic_engine.py")
...
class TestPromptForbidsCoordinateInvention:
    def _prompt_body(self, prompt_name: str) -> str:
        ...
        assert m, f"prompt {prompt_name!r} not found in semantic_engine.py"
        return m.group("body")
