...
def test_settings_tab_contains_scrollarea():
    ...
    assert len(scrolls) >= 1, "SettingsTab must contain a QScrollArea so the body is scrollable"
...
def test_lang_picker_persists_choice_to_qsettings():
    ...
    assert saved == "en", f"Switching lang picker must persist ui/language='en', got {saved!r}"
...
def test_main_window_loads_persisted_language():
    ...
    try:
        ...
        assert i18n.current_language() == "en", (
            f"MainWindow must load persisted language 'en', got {i18n.current_language()!r}"
        )
    finally:
