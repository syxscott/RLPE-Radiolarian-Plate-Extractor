...
and captions to 200 chars. LLM LLM needs the full-resolution image
...
def test_default_policy_is_api_redacted():
    ...
    paragraphs to LLM. Operators who need the historical
    ...
    ``AnthropicCompatBackend.__post_init__``)."""
...
def test_backend_default_is_api_redacted():
    ...
    field_obj = next(f for f in fields(AnthropicCompatBackend) if f.name == "data_outbound_policy")
