...
def test_jobs_tab_summary_uses_i18n():
    ...
    assert "i18n._tr" in src, (
        "jobs_tab._update_summary still uses hard-coded English f-string"
    )
    assert "{total} jobs" not in src, (
        "_update_summary still has hard-coded English '{total} jobs'"
    )
...
def test_image_preview_no_bare_english_strings():
    ...
    assert '"(no image)"' not in src, (
        "image_preview still has bare English '(no image)'"
    )
...
def test_run_tab_apply_settings_restores_ocr_backend_by_iso_code():
    ...
    assert settings["ocr_backend"] == "easyocr", (
        f"apply_settings should restore easyocr, got "
        f"{settings['ocr_backend']!r}"
    )
...
def test_image_preview_uses_i18n_no_image_key():
    ...
    assert en_text != zh_text, (
        f"image_preview no-image label did not switch: en={en_text!r} "
        f"zh={zh_text!r}"
    )
...
def test_image_preview_uses_i18n_missing_key():
    ...
    assert "(missing)" not in text, (
        f"image_preview still shows bare English '(missing)': {text!r}"
    )
...
def test_jobs_tab_summary_uses_i18n_format():
    from rlpe.gui import i18n
    from rlpe.gui.jobs_tab import JobsTab, JobRecord, STATUS_QUEUED
    jt = JobsTab()
    # Add a fake job
    job = JobRecord(
        job_id="test-job-1",
    ...
    assert en_count != zh_count, (
        f"jobs summary did not switch: en={en_count!r} zh={zh_count!r}"
    )
    ...
    for cb in rt.findChildren(QComboBox):
        if cb.itemData(0) in ("paddleocr", "easyocr"):
            assert cb.minimumHeight() >= 32 or cb.sizePolicy().verticalPolicy() == QSizePolicy.Fixed, (
                f"OCR backend combo min_height={cb.minimumHeight()}; "
                f"should be >= 32"
            )
            return
