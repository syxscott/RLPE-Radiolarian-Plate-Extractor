...
def _run_single(config: PipelineConfig, pdf_path: Path) -> list[dict[str, Any]]:
    ...
    # (``llm_per_panel`` / ``llm_stage_6`` / ``use_llm_stage3`` /
    # ``llm_multi_plate_enrich`` / ``llm_temperature`` /
    # ``llm_thinking_budget`` / ...) and each silently fell back to
    # its default when invoked through the batch path. Users who set
    # ``--llm-per-panel`` on the CLI saw the per-panel LLM path *only*
