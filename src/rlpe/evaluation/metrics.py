...
DEFAULT_GOLD_PROVENANCE = (
    "parser-derived gold: the reference set was generated by the "
    "caption parser itself (scripts/build_gold_from_captions.py), "
    "NOT image-verified by a human annotator, so these F1 figures "
    "measure parser self-consistency, not accuracy against "
    "independently annotated panels (audit 2026-09-04 eval-1; see "
    "rlpe.evaluation.gold module docstring)"
)
...
class EvaluationReport:
    ...
    # Audit 2026-09-04 eval-1: every report (Markdown AND JSON) must
    # state what the gold actually is. The default flags the
    # parser-self-consistency problem; replace it ONLY when the gold
    # really has been image-verified.
    gold_provenance: str = DEFAULT_GOLD_PROVENANCE

    def to_dict(self) -> dict[str, Any]:
        return {
            "papers": {k: v.to_dict() for k, v in self.papers.items()},
            "aggregate": dict(self.aggregate),
            "gold_provenance": self.gold_provenance,
        }
