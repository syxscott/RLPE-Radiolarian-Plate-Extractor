# LLM Per-Panel Species ID Pipeline — Feasibility Design

**Date:** 2026-08-17
**Status:** Draft — pending user approval
**Scope:** Production-pipeline integration of LLM (LLM LLM multimodal LLM) for per-panel radiolarian species naming. Goal: replace / supplement the current regex-only matching path so the system reaches the **90%+ species-level F1** target on the 9-paper gold set.

---

## 1. Background and Motivation

Today `RadiolarianPipeline._apply_stage3_bbox_crops` (pipeline.py:1721) produces per-panel crops via Stage-3 LLM vision + YOLO fallback, but **species identification** downstream still relies on:
...
PDF → GROBID / OpenDataLoader → per-figure loop:
    ...
    ↓
stage3_bbox_crops (LLM vision + YOLO → panel_path crops on disk)
    ↓
...
stage3_bbox_crops  (writes panel_path crops on disk)
    ↓
NEW: stage4_5_llm_per_panel_species_id
    ...
    - Merge into row["species"] (overwrite regex match if LLM confidence > threshold)
    - Stamp row["metadata"]["llm_per_panel"] = {raw_dict, ts, fallback_used, cost}
    ↓
...
Stage 3 (segment_panels) already produces the panel_path crops via LLM. We need those crops to do per-panel vision. Inserting Stage 4.5 *before* Stage 3 would mean paying for two vision passes per panel. Inserting *after* Stage 3 means we reuse Stage 3's crops and the YOLO fallback for papers where Stage 3 returned no panels — those rows simply skip Stage 4.5 (no crop, no per-panel call) and keep the regex-only path.

### 2.4 Where in code

- **New method on `RadiolarianPipeline`**: `_apply_llm_per_panel_species_id(results, paper_id)` — wired into the main pipeline loop in `pipeline.py` between `_apply_stage3_bbox_crops(...)` and `_apply_multi_plate_enrichment(...)`.
- **Reuses existing infrastructure**:
  - `self.semantic_engine.backend.infer_panel(...)` — already abstract 5-param method on all backends (TransformersGemmaBackend / OllamaGemmaBackend / M3Backend / LlamaCppGemmaBackend).
  - `self.config.llm_backend_concurrency` (already a semaphore, default 8 in production).
...
def _apply_llm_per_panel_species_id(
    ...
    """Stage 4.5: per-panel LLM vision species ID on Stage-3 crops.

    Falls back gracefully to regex-only when:
      * LLM backend not configured (offline mode)
      * panel_path missing or corrupted
      * backend.infer_panel raises / times out
      * LLM returns "cannot identify" with confidence < 0.3
    """
    ...
    semaphore = self.config.llm_backend_concurrency  # Semaphore(max_concurrent)
    ...
    def _one(r: dict, crop: Path, cap: str, ctx: str) -> dict | None:
        try:
            with semaphore:
                ...
                # Replace row.species only if LLM is reasonably sure
            ...
            logger.warning("LLM per-panel failed for %s/%s: %s",
                           paper_id, r.get("panel_id"), exc)
            return None

    with ThreadPoolExecutor(max_workers=self.config.llm_backend_concurrency._value) as ex:
...
1. **Confidence-gated overwrite**: LLM only wins when `confidence >= llm_per_panel_min_conf` (default 0.55). Below that, the regex match stays.
2. **Per-panel cost metadata** stamped into `metadata.llm_per_panel` for audit (which panel LLM nailed, which it couldn't).
3. **Pure additive**: rows without Stage 3 crops (e.g. YOLO fallback succeeded but no LLM-per-panel crop, or Stage 3 disabled) keep the existing species from regex.
4. **Concurrency capped at existing semaphore** (default 8 in `llm_max_concurrent`).
...
| Paper | Panels | LLM exact | LLM substring (effective) |
|-------|--------|----------|--------------------------|
...
|----------|-----------|---------------------------|-----------|
| **A — LLM only (whole-page, today)** | full page image, no panel crops | **53%** | live measurement |
| **B — LLM per-panel + caption snippet + page context** | panel crop + caption_for_this_panel + page systematic-paleontology | **70-80%** | Boughdiri-p4 88% is the upper bound (panel + caption); whole-page noise drags down to ~55% on average; per-panel context recovers most of that gap |
| **C — LLM per-panel + YOLO fallback + regex supplement** | B + fallback to regex on Stage 3 low-confidence | **80-85%** | regex catches ~25-30% of LLM misses on its own; combined ≈ 80-85% |
...
- **Lower bound** (Boughdiri-p4): 88% effective when LLM gets *exactly the right context*. **n=29 panels**, single page, so this is anecdotal.
...
- **Risk**: if Stage 3 crops are noisy (YOLO misaligned, bbox drift), per-panel LLM will see cropped-out-of-context SEM images and lose recall. Mitigated by keeping the existing path; per-panel is additive.
...
4. Aggregate per-paper consensus (vote across N=3 self-consistency samples — `llm_match_samples` config already wired)
...
Today's whole-page LLM call: **~¥0.005 per page** (1-page image + caption text prompt, ~500-800 output tokens).
...
| Usage scale | Papers/year | Per-paper cost | Annual LLM spend | Annual bandwidth (GB) |
|-------------|-------------|-----------------|------------------|------------------------|
...
Bandwidth is dominated by **image upload** to the LLM API — panel crops are smaller than pages but there are more of them. At ¥0.005/panel × ~50 panels/paper, a 1000-paper/year department-scale pipeline costs **~¥350/year**.

### 5.4 Cost-control levers (already wired)

- `llm_per_panel_enabled` (proposed, default `False` for backward compat) — opt-in
- `llm_per_panel_min_conf` (proposed, default `0.55`) — only overwrite regex when LLM is reasonably sure
- `llm_per_panel_max_per_figure` (proposed, default `20`) — cap per-figure to prevent runaway cost
- `llm_max_concurrent=8` — already throttles API rate
- Existing `llm_match_samples` (N=3 self-consistency vote) — already in `PipelineConfig`

### 5.5 Cost vs. value

¥0.40/paper × 50 panels × 100 papers = ¥2000/year = ~$280/year. Compared to a single grad-student's time at $30/hour × 8 hours/paper × 100 papers = **$24,000/year**, the **LLM spend is < 1.2% of the manual cost** for ~80% of the recall. Strong ROI.
...
Today's whole-page LLM call: **8-15 seconds per call** (dominated for image encoding + 500-token output).
...
- **Existing semaphore** `llm_max_concurrent=8` caps parallel API calls.
- 8 concurrent × 10s/call = **0.8 calls/sec/pipeline** ≈ 50 papers/hour at 30 panels each.
- Multi-process parallelism (multiple `RadiolarianPipeline` workers): scales linearly until LLM API rate-limit. Rate limit not measured precisely, but 8 concurrent already saturates a single LLM stream.

### 6.3 Async / sync

- **Synchronous today** (`backend.infer_panel` blocks). Whole pipeline is CPU-bound on regex + I/O-bound on LLM.
- **Proposed**: still synchronous (matches existing pattern); the LLM calls already use the concurrency semaphore.
...
|----------|---------------------|------------------------|
| GPU (local model path) | 0 (LLM API path doesn't need GPU) | 0 |
...
| LLM API quota | 1 call / panel | 50 calls / paper |
...
|---------|----------|----------|
| LLM API timeout (>60s) | log + skip panel | regex stays |
| LLM returns fallback_used | log + skip | regex stays |
| LLM returns low confidence (<0.55) | overwrite suppressed | row keeps regex |
| LLM returns garbage / unparseable JSON | parse_json_from_text 4-tier | regex stays |
...
**Pipeline never regresses** — Stage 4.5 is purely additive when LLM succeeds.
...
```python
llm_per_panel_enabled: bool = False  # opt-in flag, default OFF
llm_per_panel_min_conf: float = 0.55  # only overwrite regex if LLM ≥ this
llm_per_panel_max_per_figure: int = 20  # cap per-figure to prevent runaway
llm_per_panel_max_per_paper: int = 200  # cap per-paper to prevent runaway
```

### 7.2 `src/rlpe/pipeline.py`

- New method `_apply_llm_per_panel_species_id(results, paper_id)` (~80 lines, see pseudocode §3).
...
- `--llm-per-panel / --no-llm-per-panel` (default: disabled)
- `--llm-per-panel-min-conf 0.55`
- `--llm-per-panel-max-per-figure 20`
- `--llm-per-panel-max-per-paper 200`

### 7.4 `src/rlpe/web_ui/...`

(Optional, post-MVP) — add a checkbox in the Run tab "Use LLM per-panel species ID" + numeric inputs for the thresholds. Falls under existing Run-tab form schema.

### 7.5 Tests (new file: `tests/test_stage4_5_llm_per_panel.py`)

~25 unit tests covering:
1. Disabled by default — no LLM calls
...
5. Backend confidence ≥ min_conf → LLM overwrites
7. Backend raises → log + regex stays
8. Backend returns parse failure → regex stays
9. Metadata stamp present when LLM succeeds
10. Metadata stamp absent when LLM fails
...
19. Empty species from LLM → keep regex
...
|------|------------|--------|------------|
| LLM per-panel *worse* than regex on some papers | medium | recall regression | confidence-gated overwrite + cap-per-figure; live A/B before merge |
| Caption pairs not matching row panel_id | low | LLM sees wrong context | fallback to full caption + page context |
| Stage 3 crop quality degrades | low | LLM sees cropped-out noise | YOLO bbox already conservative; existing `panel_id_source` audit tag |
...
1. **Should per-panel LLM run BEFORE or AFTER `multi_plate_enrichment`?** Currently proposed after Stage 3 + before enrichment. Could move after enrichment to use enriched geology context — would be slower but possibly more accurate. Recommend: ship after Stage 3 first; revisit with A/B data.
2. **Self-consistency N=3 cost** — `llm_match_samples=3` triples cost. Recommend: ship with N=1, add as opt-in flag later.
3. **Cross-paper caching** — image SHA cache could save LLM calls if same panel image appears multiple times. Low priority — most papers have unique panels.
...
```bash
# CLI rollback
rlpe run --no-llm-per-panel …

# or in Python
config.llm_per_panel_enabled = False
```
...
- [ ] No regression on papers where LLM has 0% (Pouille-like): regex path still gives original result
- [ ] 25 new tests passing, full suite ≥1829 tests
- [ ] No new files beyond `tests/test_stage4_5_llm_per_panel.py`
...
- Building a new LLM backend / training a model
- Replacing Stage 3 with LLM per-panel (Stage 3 stays as crop producer)
- Changing schema versioning (additive only)
- Replacing regex with LLM unconditionally (always gated, always fall-back)
...
- Live LLM eval baseline: this morning's 9-paper run on `runs/real_papers_2026_08_17/` (PDFs in tree; result tables in session log 32ce786d-d497-4196-9131-ae50ea894b1e).
- Existing infrastructure: `src/rlpe/semantic_engine.py:2533` (`match_panel`), `:1784` (`PanelBox`), `:1800` (`PanelMatch`).
- Existing semaphore: `src/rlpe/config.py` `llm_max_concurrent` (default 8).
- Existing self-consistency: `llm_match_samples` (default 1).
